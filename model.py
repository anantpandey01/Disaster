...
import chart_studio.plotly as py  # ✅ Correct import
import plotly
import joblib
...
def flood_classifier(filename, fd, validating=0):
    ...
    for i in range(1, len(data1.columns)):
        data1[data1.columns[i]] = data1[data1.columns[i]].fillna(
            data1[data1.columns[i]].mean()
        )
    # In[6]:
    data1.describe()
    # In[7]:
    ...
    for i in range(len(data1["Day"])):
        if (
            data1["Day"][i] == 31
            and data1["Months"][i] == 12
            and data1["Year"][i] == 2015
        ):
            locate = i
            break
